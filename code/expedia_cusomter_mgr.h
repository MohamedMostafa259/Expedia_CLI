...
public:
	...
	CustomerManager(const CustomerManager&) = delete; // No sense for such a class (manager of objects) to be copyable!
	...
	bool MakeReservation(const Reservation& reservation, PaymentCard &payment_card) {
		...
		if (is_paid) {
			...
			if (is_confirmed) {
				...
				bool is_uncharged = expedia_manager.UnchargedCost(reservation.TotalCost(), payment_card);
